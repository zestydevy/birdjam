...
mainproc(void *arg)
{
	...
	clearcolor = ((GPACK_RGBA5551(255, 255, 255, 1) << 16) |
...
draw_centerpiece(Dynamic *dynamicp)
{
    ...
    guTranslate(&dynamicp->letters_trans, -250.0f, 0.0f, -2000.0f);
    guScale(&dynamicp->letters_scale, 0.5f, 0.5f, 0.5f);
    guRotate(&dynamicp->letters_rotate, theta, 0.2, theta, 0.0);
    ...
    gSPDisplayList(glistp++, logo_N64_Logo_mesh);
...
do_bkg( Gfx **pglistp )
{
    ...
    jam_sprite.rsp_dl_next   = jam_sprite.rsp_dl;

    spMove( &jam_sprite, 0,0 );
    spScale( &jam_sprite, 1.0, 1.0 );
    spSetAttribute( &jam_sprite, SP_FASTCOPY );

    dl = spDraw( &jam_sprite );
